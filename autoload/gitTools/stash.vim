...
function! gitTools#stash#Apply(...)
    ...
    if a:0 == 0
        echo "[gitTools.vim] Apply stash changes."
        let l:num = s:chooseStashNumber() 
    else
    ...
    if confirm("Apply changes on stash #".l:num."?", "&Yes\n&no", 2) != 1
        return
    endif

    let l:cmd = "git stash apply ".l:num
    echo l:cmd
...
function! gitTools#stash#Delete(...)
    ...
    if a:0 == 0
        echo "[gitTools.vim] Delete stash changes."
        let l:num = s:chooseStashNumber() 
    else
        let l:num = str2nr(a:1)
        if s:isStashNumAvailable(l:num) != 0
    ...
    if confirm("Delete stash #".l:num."?", "&Yes\n&no", 2) != 1
        return
    endif

    let l:cmd = "git stash drop stash@{".l:num."}"
    echo l:cmd
